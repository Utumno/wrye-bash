...
class MasterList(_ModsUIList):
    ...
    def OnLabelEdited(self, is_edit_cancelled, evt_label, evt_index, evt_item):
        ...
        if evt_label in bosh.modInfos: # evt_label is the new name
            ...
            settings[u'bash.mods.renames'][masterInfo.old_name] = evt_label
...
class ModDetails(_ModsSavesDetails):
    ...
    def DoSave(self):
        modInfo = self.modInfo
        #--Change Tests
        changeName = (self.fileStr.strip() != modInfo.ci_key)
        ...
        #--Change Name?
        if changeName:
            oldName,newName = modInfo.ci_key, self.fileStr.strip()
            #--Bad name?
...
class SaveList(balt.UIList):
    ...
    def _handle_left_down(self, wrapped_evt, lb_dex_and_flags):
        ...
        rename_res = self.try_rename(sinf, hitItem[:-len(extension)], # root
                                     ext=extension)
        if rename_res:
...
class SaveDetails(_ModsSavesDetails):
    ...
    def DoSave(self):
        ...
        to_del = set()
        if changeName:
            newName = os.path.splitext(self.fileStr.strip())[0]
